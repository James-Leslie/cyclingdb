...
class RiderSearchEngine:
    ...
    def _filter_by_specialization(
        self, df: pd.DataFrame, specialization: str
    ) -> pd.DataFrame:
        ...
        for col in df.columns:
            ...
            if any(
                stat in col_lower
                for stat in ["mountain", "sprint", "time trial", "overall", "classics"]
            ):
        ...
        return (
            df.loc[filtered_indices] if filtered_indices else df.iloc[0:0]
    ...
    def get_stats(self, filtered_df: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "teams": df_to_analyze["Team"].nunique()
            if "Team" in df_to_analyze.columns
            else 0,
            "avg_age": df_to_analyze["Age"].mean()
